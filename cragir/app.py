...
# Տեղական ադմինիստրատորի տվյալները (եթե SQL-ը չաշխատի)
...
def check_user(username, password):
    """Ստուգում է օգտատիրոջը բազայում կամ տեղական ցուցակում"""
    # 1. Ստուգում ենք տեղական ցուցակում (JSON-ից եկած)
    for u in st.session_state.users_list:
        if u["username"] == username and u["password"] == password:
            return u

    # 2. Եթե չկա, փորձում ենք Cloud SQL-ով
...
def save_to_disk():
    data = {
        ...
        "users_list": st.session_state.users_list  # Պահում ենք նաև օգտատերերին
    }
...
def load_from_disk():
    ...
    # Եթե ոչինչ չկա, դնում ենք DEFAULT_OWNER-ին
...
if "subjects" not in st.session_state:
    st.session_state.update({
        ...
        "schedule": None, "subj_pool": [], "teacher_pool": [], "users_list": [DEFAULT_OWNER]
    })
    load_from_disk()
...
# 2. ՀԱՏՈՒԿ ԷՋ՝ Օգտատերերի Կառավարում (Միայն Owner-ի համար և առանձին ներքևում)
if st.session_state.user_role == 'owner':
    if st.sidebar.button("👥 Օգտատերերի Կառավարում", width='stretch'):
        page = "👥 Օգտատերեր"

st.sidebar.divider()
# 1. Սովորական Էջերի ցուցակ (Հասանելի է բոլորին ըստ դերերի)
available_pages = ["📊 Վահանակ"]

if st.session_state.user_role in ['owner', 'subject_editor']:
    available_pages.append("📚 Առարկաներ")

if st.session_state.user_role in ['owner', 'teacher_editor']:
...
# Ցուցադրում ենք սովորական էջերը
page = st.sidebar.radio("Նավիգացիա", available_pages)
...
# --- ԷՋ 8: 👥 ՕԳՏԱՏԵՐԵՐ ---
    ...
    with col1:
        with st.form("add_user_form", clear_on_submit=True):
            ...
            new_r = st.selectbox("Դերը", ["subject_editor", "teacher_editor", "owner", "admin"])
    ...
    for i, u in enumerate(st.session_state.users_list):
        ...
        if u['username'] != st.session_state.username: # Ինքդ քեզ չջնջես
