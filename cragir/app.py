...
@st.dialog("📖 Smart Time Table: Օգտագործման Ուղեցույց")
def show_instruction_modal():
    st.markdown("""
    Բարի գալուստ **Smart Time Table**՝ AI-ով աշխատող դասացուցակի կառավարման համակարգ։ 
    Ստորև ներկայացված են հիմնական բաժինների հնարավորությունները։
    """)

    with st.expander("📊 1. Վահանակ (Dashboard)", expanded=True):
        st.markdown("""
        * **Նկարագիր.** Համակարգի «ուղեղը», որտեղ երևում է ամբողջական վիճակագրությունը։
        * **Հնարավորություն.** Տեսնում եք ուսուցիչների քանակը, դասարանների բաշխվածությունը և ակտիվ առարկաները։ Այստեղից կարող եք արագ գնահատել ծանրաբեռնվածությունը։
        """)

    with st.expander("🔐 2. Անվտանգություն և Մուտք"):
        st.markdown("""
        * **Cloud & Local.** Համակարգն աշխատում է երկակի ռեժիմով։ 
        * **Admin մուտք.** Եթե բազայի հետ կապի խնդիր լինի, Admin-ը կարող է մուտք գործել լոկալ `.env` գաղտնաբառով։
        * **Օգտատերեր.** Մնացած բոլոր օգտատերերի տվյալները պաշտպանված են **Hash** տեխնոլոգիայով և պահվում են Supabase ամպային բազայում։
        """)

    with st.expander("📚 3. Առարկաներ և Բարդություն"):
        st.markdown("""
        * **Ինչպե՞ս.** Մուտքագրեք առարկան և սահմանեք **Բարդության աստիճան (1-5)**։
        * **AI Տրամաբանություն.** Խելացի ալգորիթմը բարդ առարկաները (օր.՝ Մաթեմատիկա, Ֆիզիկա) ավտոմատ կտեղադրի օրվա 2-րդ կամ 3-րդ ժամերին, երբ աշակերտների էներգիան առավելագույնն է։
        """)

    with st.expander("👩‍🏫 4. Ուսուցիչների Կառավարում"):
        st.markdown("""
        * **Կապակցում.** Յուրաքանչյուր ուսուցչի կարող եք կցել բազմակի առարկաներ։
        * **Անձնական Էջ.** Յուրաքանչյուր ուսուցիչ ունի իր սեփական էջը, որտեղ տեսնում է միայն իր շաբաթական գրաֆիկը՝ առանց ավելորդ տեղեկատվության։
        """)

    with st.expander("🚀 5. AI Գեներացում և PDF"):
        st.markdown("""
        * **Գործընթաց.** Սեղմեք «Ստեղծել Խելացի Դասացուցակ»։ Համակարգը կստուգի ժամաքանակները, ուսուցիչների զբաղվածությունը և դասարանների համընկնումները։
        * **Արտահանում.** Պատրաստի արդյունքը կարող եք ներբեռնել բարձրորակ **PDF** ֆորմատով՝ պատրաստ տպագրության կամ տարածման։
        """)

    with st.expander("🤖 6. Gemini AI Օգնական"):
        st.markdown("""
        * **Վերլուծություն.** Կարող եք հարցնել AI-ին՝ «Ո՞ր օրն է ամենածանրը 10-Ա դասարանի համար» կամ «Ինչպե՞ս օպտիմալացնել ժամերը»։
        * **Գաղտնիություն.** Չաթի պատմությունը անձնական է և տեսանելի է միայն Ձեզ։
        """)

    st.info("☁️ **Տվյալների Պահպանում.** Բոլոր փոփոխությունները ավտոմատ կերպով համախրոնացվում են Supabase-ի հետ։ Եթե տեսնում եք «Կապի խնդիր» հաղորդագրությունը, ստուգեք ինտերնետ կապը։")
