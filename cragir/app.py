    ...
    if st.session_state.active_tab == "📊 Վահանակ":
        ...
        with col2:
            if st.session_state.teachers and st.session_state.classes:
                sel_t = st.selectbox("👩‍🏫 Ընտրեք Ուսուցչին", st.session_state.teachers, format_func=lambda x: x.name, key="sel_t_assign")
        ...
        if st.session_state.classes and st.session_state.assignments:
            selected_class_view = st.selectbox("🔍 Ընտրեք դասարանը՝ կապերը տեսնելու համար", st.session_state.classes, format_func=lambda x: f"{x.grade}{x.section}")
            ...
            if filtered_assignments:
                for i, a in filtered_assignments:
                    ...
                    if t_obj:
                        with st.container(border=True):
                            c1, c2 = st.columns([5,1])
                            c1.markdown(f"📖 **{get_subj_name(a.subject_id)}** | 👤 {t_obj.name} | <span style='color: #0d6efd;'>{a.lessons_per_week} ժամ</span>", unsafe_allow_html=True)
                ...
                with st.spinner("🧠 Ալգորիթմը մտածում է..."):
                    ...
                    for cls in shuffled_classes:
                        ...
                        while class_fund and timeout < 3000:
                            ...
                            final_schedule.append({"Դասարան": f"{cls.grade}{cls.section}", "Օր": best_day, "Ժամ": next_hour, "Առարկա": f"{get_subj_name(target.subject_id)} ({t_name})"})
        ...
        if st.session_state.get('schedule'):
            ...
            for c in df['Դասարան'].unique():
                with st.expander(f"🏫 Դասարան՝ {c}", expanded=True):
                    ...
                    cls_df['Առարկա'] = cls_df['Առարկա'].apply(lambda x: x.split(" (")[0])
                    pivot = cls_df.pivot(index='Ժամ', columns='Օր', values='Առարկա').fillna("-")
                    existing_days = [d for d in DAYS_AM if d in pivot.columns]
                    if existing_days: pivot = pivot[existing_days]
                    st.dataframe(pivot, use_container_width=True)
            pdf_bytes = generate_pdf(st.session_state.schedule)
            st.download_button(label="📥 Ներբեռնել PDF (English Only)", data=bytes(pdf_bytes), file_name="School_Timetable.pdf", mime="application/pdf", use_container_width=True, type="primary")
        ...
        if current_user not in st.session_state.chat_histories: st.session_state.chat_histories[current_user] = []
        ...
        if st.session_state.pending_proposal:
            with st.chat_message("assistant"):
                st.warning("💡 AI-ն ունի առաջարկ։ Կիրառե՞լ այն։")
                col_yes, col_no = st.columns(2)
                if col_yes.button("✅ Կիրառել", use_container_width=True):
                    proposal_text = st.session_state.pending_proposal
                    st.session_state.pending_proposal = None
                    try:
                        context = "Դու 'Smart Time Table' AI օգնականն ես։ Օգտատերը ՀԱՄԱՁԱՅՆԵՑ առաջարկին։ Արա փոփոխությունը և ցույց տուր Markdown աղյուսակով։\n"
                        if st.session_state.schedule: context += f"Բազա՝ {json.dumps(st.session_state.schedule, ensure_ascii=False)}\n"
                        client = genai.Client(api_key=st.secrets["GEMINI_API_KEY"])
                        response = client.models.generate_content(model='gemini-2.0-flash', contents=context + f"\nԱռաջարկ՝ {proposal_text}")
                ...
                if col_no.button("❌ Չեղարկել", use_container_width=True):
                    st.session_state.pending_proposal = None; st.rerun()

        if prompt := st.chat_input("Ինչպե՞ս կարող եմ օգնել։"):
            st.session_state.chat_histories[current_user].append({"role": "user", "content": prompt})
            with st.chat_message("user"): st.markdown(prompt)
            with st.chat_message("assistant"):
                with st.spinner("🧠..."):
                    try:
                        context = "Դու 'Smart Time Table' AI օգնականն ես։\n"
                        if st.session_state.schedule: context += f"Դասացուցակ՝ {json.dumps(st.session_state.schedule, ensure_ascii=False)}\n"
                        client = genai.Client(api_key=st.secrets["GEMINI_API_KEY"])
                        response = client.models.generate_content(model='gemini-2.0-flash', contents=context + f"Հարց՝ {prompt}")
                        res_text = response.text
                        if any(x in res_text.lower() for x in ["փոխել", "swap", "տեղափոխ"]): st.session_state.pending_proposal = res_text
                    except Exception as e: res_text = f"Սխալ՝ {str(e)}"
